...
class Escrow(sp.Contract):
    ...
    @sp.entry_point
    def createTransaction(self, transactionId=sp.none):
    ...
    @sp.entry_point
    def setTransactionDetails(self, owner, transactionId, counterparty, duration, offer, fee, secret, description, title):
        ...
        transaction.active = sp.bool(False)

    @sp.entry_point
    def setTransactionActive(self, transactionId, active):
        sp.set_type(transactionId, sp.TString)
        sp.verify(self.data.transactions.contains(transactionId),
                  "Transaction does not exist.")
        sp.verify(sp.sender == self.data.master,
    ...
    @sp.entry_point
    def setTransactionParticipants(self, transactionId, owner, counterparty):
        sp.set_type(transactionId, sp.TString)
        sp.verify(self.data.transactions.contains(transactionId),
                  "Transaction does not exist.")
        sp.verify(sp.sender == self.data.master,
    ...
    def revertCommissionFunds(self, transactionId):
        ...
        sp.verify(self.data.transactions[transactionId].ownerHasWithdrawn == sp.bool(True),
                  "Owner did not cancel the commission.")
        sp.verify(self.data.transactions[transactionId].counterpartyHasWithdrawn == sp.bool(True),
    ...
    def postCommission(self, offer, fee, duration, secret, description, title, transactionId=sp.none, ):
        ...
        with sp.if_(transactionId == sp.none):
            ...
            self.data.parties[newTransactionId].owner = sp.some(
                sp.sender)
    ...
    @sp.entry_point
    def activateTransaction(self, transactionId):
        sp.set_type(transactionId, sp.TString)
        sp.verify(self.data.transactions.contains(transactionId),
                  "Transaction does not exist.")
        ...
        transaction = self.data.transactions[transactionId]
        sp.verify(transaction.active == sp.bool(False),
                  "Transaction is already active!")
        sp.verify(~transaction.withdrawn,
        ...
        transaction.active = sp.bool(True)
    ...
    def depositOwner(self, transactionId):
        ...
        sp.verify(self.data.transactions.contains(transactionId),
                  "Transaction does not exist.")
        sp.verify(~self.data.transactions[transactionId].active,
                  "Transaction is active.")
        sp.verify(~self.data.transactions[transactionId].withdrawn,
                  "Transaction has been withdrawn.")
        sp.verify(self.data.parties[transactionId].owner == sp.some(sp.sender),
    ...
    def depositCounterparty(self, transactionId):
        ...
        sp.verify(self.data.transactions[transactionId].balanceCounterparty == sp.tez(0),
                  "Counterparty already deposited funds.")
        sp.verify(sp.amount == self.data.transactions[transactionId].fee,
    ...
    def approveCommission(self, transactionId):
        ...
        transaction = self.data.transactions[transactionId]
        sp.verify(transaction.active == sp.bool(True),
                  "Transaction not active.")
        sp.verify(transaction.withdrawn == sp.bool(False),
                  "Transaction already withdrawn.")
    ...
    def claim(self, identity, transactionId):
        ...
        sp.verify(transaction.withdrawn == sp.bool(False),
                  "Transaction already withdrawn.")
        sp.verify(~transaction.withdrawn, "Escrow already withdrawn!")
        ...
        transaction.withdrawn = sp.bool(True)
...
def test():
    ...
    scenario += c1.createTransaction(sp.some("first")).run(sender=admin)
    scenario += c1.setTransactionDetails(
        title="Cat caretaker",
        owner=owner,
    ...
    scenario += c1.setTransactionActive(
        transactionId="first",
        active=sp.bool(False)
    ).run(sender=admin)

    scenario.h2("Set participants")
    scenario += c1.setTransactionParticipants(
        owner=owner,
        counterparty=counterparty,
        transactionId="first"
    ...
    scenario += c1.activateTransaction(
        "first"
